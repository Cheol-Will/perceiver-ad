...
def train_test(args, run, model_config):
    ...
    if model_config['model_type'] in DEEP_MODELS: 
        trainer.training(model_config['epochs'])
    elif model_config['model_type'] in CLASSIC_MODELS: 
        trainer.training()
    else:
        raise ValueError(f"Unknown model_type is given: {model_config['model_type']}.")
...
def main(args):
    ...
    all_results = []
    for i in range(10):
        result = train_test(args, i, model_config)
