...
from models.MOCO.Model import MOCO
...
class Trainer(object):
    def __init__(self, model_config: dict, train_config: dict):
        ...
        print(f"="*60)
        print(f"MOCO Model Configuration:")
        ...
        self.model = MOCO(**model_config).to(self.device)
            ...
            for step, (x_input, y_label, sample_indices) in enumerate(self.train_loader):
                ...
                if self.use_amp:
                    with autocast():
                        ...
                        loss = output['loss']
                        # Handle both scalar and tensor loss
    ...
    def evaluate(self):
        """
        Evaluate model using reconstruction-based anomaly scores.
        """
        model = self.model
        model.eval()
        ...
        avg_normal_score = score[test_label == 0].mean() if (test_label == 0).sum() > 0 else 0.0
        avg_abnormal_score = score[test_label == 1].mean() if (test_label == 1).sum() > 0 else 0.0
        ...
        print(f"[Eval] Test samples: {len(test_label)}")
        return metric_dict
