...
from models.TKDAD.Model import TKDAD
...
class Trainer(object):
    ...
    def training(self):
        ...
        for epoch in range(self.epochs):
            ...
            for step, (x_input, y_label) in enumerate(self.train_loader):
                ...
                if self.use_amp:
                    with autocast():
                        reg_loss, contra_loss, _ = self.model(x_input)
                        reg_mean = reg_loss.mean()
                        contra_mean = contra_loss.mean()
                        loss = reg_mean + (contra_mean * self.model.contrastive_loss_weight)
                ...
                running_reg_loss += reg_mean.item()
                running_contra_loss += contra_mean.item()

            scheduler.step()
            
            avg_loss = running_loss / len(self.train_loader)
            avg_reg_loss = running_reg_loss / len(self.train_loader)
            avg_contra_loss = running_contra_loss / len(self.train_loader)
            
            info = 'Epoch:[{}]\t loss={:.6f}\t reg_loss={:.6f}\t contra_loss={:.6f}'
            self.logger.info(info.format(epoch, avg_loss, avg_reg_loss, avg_contra_loss))
            self._log_training(epoch, avg_loss, avg_reg_loss, avg_contra_loss)
    ...
    def evaluate(self):
        ...
        for step, (x_input, y_label) in enumerate(self.test_loader):
            x_input = x_input.to(self.device)
            _, _, anomaly_score = model(x_input)
            
            score.append(anomaly_score.cpu())
        ...
        avg_normal_score = score[test_label == 0].mean() if len(score[test_label == 0]) > 0 else 0.0
        avg_abnormal_score = score[test_label == 1].mean() if len(score[test_label == 1]) > 0 else 0.0
    ...
    def _log_training(self, epoch, avg_loss, avg_reg_loss, avg_contra_loss):
        if self.writer:
            self.writer.add_scalars(f"{self.dataname}/Loss/Total", 
                {f'Run_{self.run}': avg_loss}, epoch)
            
            self.writer.add_scalars(f"{self.dataname}/Loss/Regression", 
                {f'Run_{self.run}': avg_reg_loss}, epoch)

            self.writer.add_scalars(f"{self.dataname}/Loss/Contrastive", 
                {f'Run_{self.run}': avg_contra_loss}, epoch)
