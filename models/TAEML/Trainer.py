...
from models.TAEML.Model import TAEML
...
class Trainer(object):
    def __init__(self, model_config: dict, train_config: dict):
        ...
        self.model = TAEML(**model_config).to(self.device)
    ...
    def training(self):
        ...
        for epoch in range(self.epochs):
            ...
            info = 'Epoch:[{}]\t loss={:.6f}\t recon_loss={:.6f}'
            self.logger.info(info.format(epoch, avg_loss, avg_recon_loss))
            self._log_training(epoch, avg_loss, avg_recon_loss)
    ...
    def _log_training(self, epoch, avg_loss, avg_recon_loss):
        if self.writer:
