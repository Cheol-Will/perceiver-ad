...
from models.TAEIMIX.Model import TAEIMIX
...
class Trainer(object):
    def __init__(self, model_config: dict, train_config: dict):
        ...
        self.model = TAEIMIX(**model_config).to(self.device)
    ...
    def training(self):
        ...
        for epoch in range(self.epochs):
            ...
            info = 'Epoch:[{}]\t loss={:.6f}\t recon_loss={:.6f}\t imix_loss={:.6f}'
    ...
    def evaluate(self):
        ...
        imix_score_list, combined_score_list = [], []
        ...
        for step, (x_input, y_label) in enumerate(self.test_loader):
            ...
            imix_score = output['imix_score'].data.cpu()
            combined = output['combined'].data.cpu()

            score_list.append(recon)
            imix_score_list.append(imix_score)
        ...
        imix_score = torch.cat(imix_score_list, axis=0).numpy()
        ...
        metric_dict.update(calc_metrics(imix_score, test_label, prefix='imix_'))
        metric_dict.update(calc_metrics(combined_score, test_label, prefix='combined_'))
        return metric_dict

    def _log_training(self, epoch, avg_loss, avg_recon_loss, avg_imix_loss):
        if self.writer:
            ...
            self.writer.add_scalars(f"{self.dataname}/Loss/IMIX", {f'Run_{self.run}': avg_imix_loss}, epoch)
            self.writer.flush()

    def _log_evaluation(self, epoch, metrics):
        if self.writer:
            score_types = {'Recon': '', 'IMIX': 'imix_', 'Combined': 'combined_'}
