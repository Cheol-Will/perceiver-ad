...
class PAESupervised(nn.Module):
    def __init__(
        ...
        self.proj = OutputProjection(1, hidden_dim) # only one output
    ...
    def forward(self, x, return_pred = False, return_weight = False):
        ...
        decoder_query = self.decoder_query.expand(batch_size, -1, -1) # (B, 1, D)
        output, decoder_attn = self.decoder(decoder_query, latents, latents, return_weight=True)
        y_pred = self.proj(output) # (B, 1) assume regression task

        if return_weight:
            return y_pred, encoder_attn, self_attns, decoder_attn
        else:
            return y_pred