...
from models.TAEDACL.Model import TAEDACL
...
class Trainer(object):
    def __init__(self, model_config: dict, train_config: dict):
        ...
        self.model = TAEDACL(**model_config).to(self.device)
    ...
    def training(self):
        ...
        for epoch in range(self.epochs):
            ...
            for step, (x_input, y_label) in enumerate(self.train_loader):
                ...
                running_ssl_loss += ssl_loss.item()

            scheduler.step()

            avg_loss = running_loss / len(self.train_loader)
            avg_recon_loss = running_recon_loss / len(self.train_loader)
            avg_ssl_loss = running_ssl_loss / len(self.train_loader)

            info = 'Epoch:[{}]\t loss={:.6f}\t recon_loss={:.6f}\t byol_loss={:.6f}'
            self.logger.info(info.format(epoch, avg_loss, avg_recon_loss, avg_ssl_loss))
            self._log_training(epoch, avg_loss, avg_recon_loss, avg_ssl_loss)
            ...
            if (epoch + 1) % self.eval_interval == 0:
                ...
                score_configs = [('Recon', ''), ('BYOL', 'byol_'), ('Combined', 'combined_')]
    ...
    def evaluate(self):
        ...
        byol_score_list, combined_score_list = [], []

        for step, (x_input, y_label) in enumerate(self.test_loader):
            x_input = x_input.to(self.device)
            output = model(x_input)

            recon = output['reconstruction_loss'].data.cpu()
            byol_score = output['byol_score'].data.cpu()
            combined = output['combined'].data.cpu()

            score_list.append(recon)
            byol_score_list.append(byol_score)
        ...
        byol_score = torch.cat(byol_score_list, axis=0).numpy()
        ...
        metric_dict.update(calc_metrics(byol_score, test_label, prefix='byol_'))
        metric_dict.update(calc_metrics(combined_score, test_label, prefix='combined_'))
        return metric_dict

    def _log_training(self, epoch, avg_loss, avg_recon_loss, avg_byol_loss):
        if self.writer:
            self.writer.add_scalars(f"{self.dataname}/Loss/Total", {f'Run_{self.run}': avg_loss}, epoch)
            self.writer.add_scalars(f"{self.dataname}/Loss/Reconstruction", {f'Run_{self.run}': avg_recon_loss}, epoch)
            self.writer.add_scalars(f"{self.dataname}/Loss/BYOL", {f'Run_{self.run}': avg_byol_loss}, epoch)
            self.writer.flush()

    def _log_evaluation(self, epoch, metrics):
        if self.writer:
            score_types = {'Recon': '', 'BYOL': 'byol_', 'Combined': 'combined_'}
