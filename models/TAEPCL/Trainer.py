...
from models.TAEPCL.Model import TAEPCL
...
class Trainer(object):
    def __init__(self, model_config: dict, train_config: dict):
        ...
        self.model = TAEPCL(**model_config).to(self.device)
            ...
            for step, (x_input, y_label) in enumerate(self.train_loader):
                ...
                if self.use_amp:
                    with autocast():
                        ...
                        contra_loss = output['contrastive_loss'].mean()  
    ...
    def evaluate(self):
        ...
        for step, (x_input, y_label) in enumerate(self.test_loader):
            ...
            contra_score_list.append(contra)
        ...
        contra_metrics = calc_metrics(contra_score, test_label, prefix='contra_')
        metric_dict.update(contra_metrics)
        combined_metrics = calc_metrics(combined_score, test_label, prefix='combined_')
        metric_dict.update(combined_metrics)
    ...
    def _log_training(self, epoch, avg_loss, avg_recon_loss, avg_contra_loss):
        ...
        if self.writer:
            ...
            self.writer.add_scalars(f"{self.dataname}/Loss/Reconstruction", 
                {f'Run_{self.run}': avg_recon_loss}, epoch)
