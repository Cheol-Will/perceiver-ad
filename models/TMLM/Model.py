...
class TMLM(nn.Module):
    ...
    def _generate_fixed_mask(self, x):
        batch_size, num_features = x.shape
        len_keep = int(num_features * (1 - self.mask_ratio))
        
        if self.share_mask:
            noise = torch.rand(1, num_features, device=x.device)
            ids_shuffle = torch.argsort(noise, dim=1) # (1, F)
            ids_keep = ids_shuffle[:, :len_keep] # (1, K)
            ids_keep = ids_keep.expand(batch_size, -1) # (B, K)
        else:
            noise = torch.rand(batch_size, num_features, device=x.device)
            ids_shuffle = torch.argsort(noise, dim=1) # (B, F)
    ...
    def forward(self, x, return_dict = False):
        if self.training:
            mask = self._generate_fixed_mask(x)
            ...
            for start_idx in range(0, R, chunk_size):
                ...
                z_chunk, _ = self.encoder(x_expanded, mask_expanded) # (B*r, D)
