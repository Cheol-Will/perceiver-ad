...
from models.ICL.Model import ICL
...
class Trainer(object):
    def __init__(self, model_config: dict, base_path: str):
        ...
        self.model = ICL(model_config).to(self.device)
    ...
    def training(self, epochs):
        ...
        # torch.save(self.model, os.path.join(self.base_path, "DRL.pth"))
        # torch.save(self.model, f"./models/{self.model_config['dataset_name']}_DRL.pth")
        print("Training complete.")
        train_logger.handlers.clear()

    def evaluate(self):
        # model = torch.load(self.model, os.path.join(self.base_path, "DRL.pth"))
        # model = torch.load(f"./models/{self.model_config['dataset_name']}_DRL.pth")
        model = self.model
        model.eval()
