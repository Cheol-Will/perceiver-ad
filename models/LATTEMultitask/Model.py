...
class LATTEMultitask(nn.Module):
    def __init__(
        self,
        ...
        super(LATTEMultitask, self).__init__()
        # dataset-specific parameters
        self.pos_encoding_list = nn.ParameterList(
            [nn.Parameter(torch.empty(1, num_features, hidden_dim)) for num_features in num_features_list]
        )
        self.feature_tokenizer_list = nn.ModuleList(
            [FeatureTokenizer(num_features, hidden_dim) for num_features in num_features_list]
        )
        self.proj = nn.ModuleList(
            [OutputProjection(num_features, hidden_dim) for num_features in num_features_list]
        )  
        
        # shared-across-dataset parameters        
    ...
    def reset_parameters(self):
        ...
        for parameter in self.pos_encoding_list:
    ...
    def forward(self, x, dataset_idx):    
        ...
        # feature tokenizer
        feature_embedding = self.feature_tokenizer_list[dataset_idx](x) # (B, F, D)
        feature_embedding = feature_embedding + self.pos_encoding_list[dataset_idx]
        ...
        if self.is_weight_sharing:
            ...
            for _ in range(self.depth):
                latents, _ = self.block(latents)

        latents_hat, _ = self.memory(latents) # (B, N, D), (B, N, M) 
        decoder_query = self.pos_encoding_list[dataset_idx].expand(batch_size, -1, -1)
        output, _ = self.decoder(decoder_query, latents_hat, latents_hat, return_weight=True)    
        x_hat = self.proj[dataset_idx](output)
