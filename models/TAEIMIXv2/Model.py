...
class TAEIMIXv2(nn.Module):
    def __init__(
        ...
        self.contra_loss_weight = float(contra_loss_weight)
    ...
    def _sample_lam(self, batch_size: int, device):
    ...
    def _imix_views(self, x):
        batch_size = x.shape[0]
        device = x.device

        perm = torch.randperm(batch_size, device=device)
        lam = self._sample_lam(batch_size, device)

        x_aug = lam * x + (1.0 - lam) * x[perm]

        return x_aug, lam, perm

    def imix_loss(self, p1, p2, lam, perm):
        batch_size = p1.shape[0]
        logits = torch.matmul(p2, p1.T) / self.temperature
        ori = torch.arange(batch_size).to(p1.device)
        ce0 = F.cross_entropy(logits, ori, reduction="none")
    ...
    def forward(self, x):
        ...
        if self.training:
            x_aug, lam, perm = self._imix_views(x)
            ...
            contra_loss = self.imix_loss(p_norm, p_aug_norm, lam, perm)
            contra_loss = self.contra_loss_weight * contra_loss
            loss = recon_loss + contra_loss # (B, )
