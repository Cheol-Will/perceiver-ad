...
class PVAE(nn.Module):
    def __init__(
        ...
        print(f"Init PVAE with beta={beta}")
        ...
        self.fc_mu = nn.Linear(hidden_dim*num_latents, hidden_dim)
        self.fc_var = nn.Linear(hidden_dim*num_latents, hidden_dim)
        self.decoder_input = nn.Linear(hidden_dim, hidden_dim*num_latents)
    ...
    def _encode(self, x):
        ...
        latents = latents.view(batch_size, -1) # (B, ND)
        mu = self.fc_mu(latents) # (B, D)
        log_var = self.fc_var(latents) # (B, D)
    ...
    def forward(self, x):
        # decoder
        num_repeats = 1 if self.training else 30
        ...
        for _ in range(num_repeats):
            ...
            z = self.decoder_input(z).view(batch_size, self.num_latents, -1)
