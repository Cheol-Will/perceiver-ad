...
from models.NEPA.Model import NEPA_TAD
...
class Trainer(object):
    def __init__(self, model_config: dict, train_config: dict):
        ...
        self.model = NEPA_TAD(**model_config).to(self.device)
        ...
        # NEPA specific
        self.num_permutations = model_config.get('num_permutations', 30)
        
        print(f"patience={self.patience} with min_delta={self.min_delta}")
        print(f"eval_interval={self.eval_interval}")
        print(f"num_permutations={self.num_permutations}")
    ...
    def evaluate(self):
        ...
        for step, (x_input, y_label) in enumerate(self.test_loader):
            x_input = x_input.to(self.device)
            # Permutation ensemble for inference
            loss = model.compute_anomaly_score(
                x_input,
