...
from models.OELATTE.Model import OELATTE
...
class Trainer(object):
    def __init__(self, model_config: dict, train_config: dict):
        ...
        if train_config['latent_ratio'] is not None:
            model_config['num_latents'] = int(model_config['num_latents']*train_config['latent_ratio'])
        ...
        if model_config['num_memories'] is None:
            ...
            if train_config['memory_ratio'] is not None:
                model_config['num_memories'] = int(model_config['num_memories']*train_config['memory_ratio'])
    ...
    def _log_training(self, epoch, avg_loss):
        ...
        if self.writer:
            # dataname으로 구분, Loss 플롯에 run들 모음
            self.writer.add_scalars(f"{self.dataname}/Loss", 
    ...
    def _log_evaluation(self, epoch, metrics):
        ...
        if self.writer:
            self.writer.add_scalars(f"{self.dataname}/RAUC", 
                {f'Run_{self.run}': metrics['rauc']}, epoch)
            
            self.writer.add_scalars(f"{self.dataname}/AP", 
                {f'Run_{self.run}': metrics['ap']}, epoch)
            
            self.writer.add_scalars(f"{self.dataname}/F1", 
                {f'Run_{self.run}': metrics['f1']}, epoch)
            
            self.writer.add_scalars(f"{self.dataname}/Score_Normal", 
                {f'Run_{self.run}': metrics['avg_normal_score']}, epoch)
            
            self.writer.add_scalars(f"{self.dataname}/Score_Abnormal", 
