...
from models.PAEKNN.Model import PAEKNN
...
class Trainer(object):
    ...
    def evaluate(self):
        model = self.model
        model.eval()
        memory_bank = model.calculate_memory_bank(self.train_loader, self.device)
        score, test_label = [], []
        
        for step, (x_input, y_label) in enumerate(self.test_loader):
            x_input = x_input.to(self.device)
            loss = model.retrieve(x_input, memory_bank, k=5) # default k = 5 (following KNN)
            loss = loss.data.cpu()
            score.append(loss)
