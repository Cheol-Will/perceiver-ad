...
class PAEKNN(nn.Module):
    ...
    @torch.no_grad()
    def calculate_memory_bank(self, train_loader, device):
        memory_bank = []        
        for (x, y) in train_loader:
            x = x.to(device)
            latents = self._encode(x) # (B, ND)
            memory_bank.append(latents)
    ...
    def _encode(self, x):
        ...
        latents = latents.view(batch_size, -1) # (B, N, D) -> (B, N*D)
        
        return latents

    @torch.no_grad()
    def retrieve(self, x, memory_bank, k: int = 5):
        '''
        retrieve top-k neighbors and calculate the average distance.
        '''
        x = self._encode(x) # (B, ND)
        batch_size, dim = x.shape
        device = x.device

        if self.search_index is None:
            self.search_index = (
                faiss.GpuIndexFlatL2(faiss.StandardGpuResources(), dim)
                if device.type == 'cuda'
                else faiss.IndexFlatL2(dim)
            )
        self.search_index.reset()
        self.search_index.add(memory_bank)

        # In memory bank, we only have train samples.
        distances, idx = self.search_index.search(x, k) # (B, K), (B, K)
        distances = distances.mean(dim=-1) # (B)
