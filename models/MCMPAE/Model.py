...
import math
import random
from models.MCMPAE.MaskNets import MultiNets, Generator
from models.MCMPAE.Score import LossFunction, DiversityMask, ScoreFunction
...
class MCMPAE(nn.Module):
    def __init__(
        ...
        use_mask_token: bool = False,
        use_pos_enc_as_query: bool = False,
        mask_generator_depth: int = 3,
        mask_num: int = 15,
        diversity_loss_weight: float = None,
    ):
        assert diversity_loss_weight is not None
        super().__init__()
        self.maskmodel = Generator(
            MultiNets(), 
            num_features, 
            mask_generator_depth,
            mask_num,
        )
        self.encoder_decoder = PAE(
            num_features,
            num_heads,
            depth, 
            hidden_dim,
        ...
        self.loss_fun = LossFunction(mask_num, diversity_loss_weight) # takes [B, M, F] as inputs
        self.score_fun = ScoreFunction(mask_num)


    def forward(self, x):

        x_mask, masks = self.maskmodel(x)
        batch_size, num_masks, num_features = x_mask.shape
        x_mask = x_mask.reshape(batch_size*num_masks, num_features)  # [B, M, F] -> [BM, F]
        x_recon = self.encoder_decoder(x_mask)
        x_recon = x_recon.reshape(batch_size, num_masks, num_features) # [B, M, F]
        
        if self.training:
            loss, mse, divloss = self.loss_fun(x, x_recon, masks)
            return loss, mse, divloss
        else:
            mse_batch = self.score_fun(x, x_recon)
            return mse_batch