...
class LossFunction(nn.Module):
    def __init__(
        ...
        self.lamb = diversity_loss_weight

    def forward(self, x_input, x_pred, masks):
        x_input = x_input.unsqueeze(1).repeat(1, self.mask_num, 1)
        sub_result = x_pred - x_input  # (B, M, F)
        mse = torch.norm(sub_result, p=2, dim=2) # B, M
        mse_score = torch.mean(mse, dim=1, keepdim=False) # (B)
        divloss = self.divloss(masks) # (B,)
        loss = mse_score + self.lamb*divloss
        return loss, mse_score, divloss
...
class ScoreFunction(nn.Module):
    ...
    def forward(self, x_input, x_pred):
        x_input = x_input.unsqueeze(1).repeat(1, self.mask_num, 1) # B, M, F
