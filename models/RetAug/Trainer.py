...
from models.RetAug.Model import RetAug
...
class Trainer(object):
    def __init__(self, model_config: dict, train_config: dict):
        ...
        self.model = RetAug(**model_config).to(self.device)
        ...
        self.use_mask_bank = train_config.get('use_mask_bank', False)
    ...
    def _cache_train_data(self):
        ...
        # Keep only normal samples (label = 0) for support set
        normal_mask = self.train_labels_full == 0
        self.train_data_normal = self.train_data_full[normal_mask]
        
        self.logger.info(f"Cached training data: {self.train_data_full.shape}")
        self.logger.info(f"Normal samples for support: {self.train_data_normal.shape}")

    def _sample_support_set(self, num_samples=None):
        """Randomly sample support set from normal training data"""
        if num_samples is None:
            num_samples = self.num_context_samples
        
        N_train = len(self.train_data_normal)
        num_samples = min(num_samples, N_train)
        
        indices = torch.randperm(N_train)[:num_samples]
        support_data = self.train_data_normal[indices]
    ...
    def evaluate(self):
        ...
        if not hasattr(self, 'train_data_normal'):
            self._cache_train_data()
            
        # Use all normal training data as support set
        x_support = self.train_data_normal.to(self.device)
