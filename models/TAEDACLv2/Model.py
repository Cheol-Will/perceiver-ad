...
class TAEDACLv2(nn.Module):
    ...
    def simclr_loss(self, p1, p2):
        # (B, D)
        logits = torch.matmul(p1, p2.T) / self.temperature # (B, B)
        labels = torch.arange(p1.shape[0]).to(p1.device)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            contra_loss1 = self.simclr_loss(p_norm, p_aug_norm)
            contra_loss2 = self.simclr_loss(p_aug_norm, p_norm)
            contra_loss = contra_loss1 + contra_loss2
