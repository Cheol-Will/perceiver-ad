...
class BiDiectionalCrossAttention(nn.Module):
    def __init__(
        self,
        hidden_dim: int,
        num_heads: int,
        ...
        super(BiDiectionalCrossAttention, self).__init__()
        self.latents_attneds_to_mask = CrossAttention(hidden_dim, num_heads, mlp_ratio, dropout_prob)
        self.mask_attneds_to_latents = CrossAttention(hidden_dim, num_heads, mlp_ratio, dropout_prob)

    def forward(self, latents, mask):
        latents_ = self.latents_attneds_to_mask(latents, mask, mask)
        mask_ = self.mask_attneds_to_latents(mask, latents, latents)
...
class RIN(nn.Module):
    ...
    def forward(self, x):
        ...
        for target_col_idx in target_col_idx_list:
            ...
            latents = self.feature_tokenizer(x, input_col_idx) # (B, F', D)
            latents = latents + self.pos_encoding[:, input_col_idx, :]
            ...
            loss_mask = F.mse_loss(pred_mask, target_mask, reduction='none').mean(dim=1) # keep batch dim
            batch_losses = batch_losses + loss_latents + loss_mask # (B)
