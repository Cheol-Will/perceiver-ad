...
data_list=(campaign mammography nslkdd) # from MCM
...
for data in "${data_list[@]}"; do
    for hidden_dim in "${hidden_dim_list[@]}"; do
        for learning_rate in "${learning_rate_list[@]}"; do
            ...
            exp_name="$model_type-ws-d$hidden_dim-lr$learning_rate-log"
            python main.py \
                --dataname "$data" \
...
# data_list=(census fraud "satimage-2" shuttle) # from MCM
