    ...
    for hidden_dim in "${hidden_dim_list[@]}"; do
        for learning_rate in "${learning_rate_list[@]}"; do
            for num_repeat in "${num_repeat_list[@]}"; do
                echo "Running $model_type data=$data dim=$hidden_dim learning_rate=$learning_rate num_repeat=$num_repeat"
                exp_name="$model_type-d$hidden_dim-lr$learning_rate-r$num_repeat"
                python main.py \
                    --dataname "$data" \
